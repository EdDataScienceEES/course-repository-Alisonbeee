...
# we don’t really need that grey background and people know that on a map you have latitude and longitude as the axes.
(lter_map4 <- ggplot() +
    geom_map(map = north_america, data = north_america,
             aes(long, lat, map_id = region), 
             color = "gray80", fill = "gray80", size = 0.3) +
...
# So we can add a label just for one of the sites, Niwot Ridge, from where the plant data for the rest of the tutorial comes.
(lter_map7 <- ggplot() +
    geom_map(map = north_america, data = north_america,
             aes(long, lat, map_id = region), 
             color = "gray80", fill = "gray80", size = 0.3) +
...
#  we can also add some text with an interesting fact about the site.
(lter_map8 <- ggplot() +
    geom_map(map = north_america, data = north_america,
             aes(long, lat, map_id = region), 
             color = "gray80", fill = "gray80", size = 0.3) +
...
# - how many times during the field data collection the pin used for observations “hit” a Carex species.
...
# When we want to plot data from different data frames in the same graph, 
# we have to move the data frame from the main ggplot() call to the specific part of 
# the graph where we want to use each dataset. Compare the code below with the code for the previous versions of the histograms to spot the difference.
...
# Now we can explore how bird population trends vary across different feeding strategies. 
# The graphs below are all different ways to answer the same question. Have a ponder about which graph you like the most.
...
# Finally, we can also use geom_segment to connect the points for the mean trends 
# to the line for the overall mean, so we can judge how far off each category is from the mean.
(trends_diet <- ggplot() +
...
# Knowing the sample size for each diet category is another useful bit of information, especially to support the spirit of open and transparent science. 
# We can use group_by() and tally() to get the sample size numbers.
